...
export default {
  filename: 'bedlamite_blank_v1.pdf',
  fields: {
    ...
    ref: { x: 105, y: 595, calc: (scores: Stats) => add_plus(ability_modifier(scores['agl'], 1)) },
    fort: { x: 105, y: 550, calc: (scores: Stats) => add_plus(ability_modifier(scores['sta'])) },
    will: { x: 105, y: 505, calc: (scores: Stats) => add_plus(ability_modifier(scores['per'], 1)) },
    grit: {
    ...
    weapon: {
      x: 150, y: 447, calc: (scores: Stats) => weapon(scores),
      style: { size: 6 }
    },
    firearm: {
    ...
    damage: {
      x: 205, y: 447, calc: (scores: Stats) => damage(scores),
      style: { size: 6 }
    },
    // trade_good: [{
    //   x: 150, y: 450, calc: (scores: Stats) => {
    //     const trade_good = occupations[scores['occupation']].trade_good
    //     return trade_good.substring(0, trade_good.indexOf('(') - 1)
    //   }
    // }],
